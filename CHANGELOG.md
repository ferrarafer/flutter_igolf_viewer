...
* fix(android): 16 KB page-size support for the viewer's native triangulator. Google Play flagged `lib/arm64-v8a/libtriangulation.so` ("Your app could crash on 16 KB devices"): iGolf's prebuilt was NDK r16, and its RELRO segment ends on a 4 KB boundary, so on 16 KB-page kernels the loader's read-only protection can cover live `.data`. The `jni/*/libtriangulation.so` entries in `iGolfViewerStandard.aar` are replaced with an NDK r29 rebuild; everything else in the AAR (`classes.jar`, resources, manifest) is byte-identical. The library is the SGI GLU tessellator plus a small JNI wrapper, and its source and build script now live in `viewer_sample_app` (`3dviewer/src/main/cpp/triangulation`, `scripts/build-triangulation.sh`). Output is bit-identical to the old library on 5,668 real-course polygon variants and 3,000 fuzzed contours. The `armeabi` and `mips` ABIs are dropped (modern NDKs can't target them, and they were never packaged into the app).
